...
class yoloRotatebbox:
    def __init__(self, filename, image_ext, angle):
        assert os.path.isfile("Images/" + filename + image_ext)
        assert os.path.isfile("Images/" + filename + '.txt')
        ...
        self.image = cv2.imread("Images/" + self.filename + self.image_ext, 1)
    ...
    def rotateYolobbox(self):
        ...
        f = open("Images/" + self.filename + '.txt', 'r')
...
if __name__ == "__main__":
	...
	# New function --> if you wanna rotate an entire folder of images
    ...
    for image in os.listdir(dir_path): 
        if image.endswith(".jpg"):
            ...
            cv2.imwrite('RotatedImages/' + 'rotated_'+image_name+'_' + str(angle) + '.jpg', image)

            file_name = 'RotatedImages/'+'rotated_'+image_name+'_' + str(angle) + '.txt'
